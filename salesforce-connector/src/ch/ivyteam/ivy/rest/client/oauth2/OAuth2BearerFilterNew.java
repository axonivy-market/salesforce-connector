...
public class OAuth2BearerFilterNew implements javax.ws.rs.client.ClientRequestFilter {
  ...
  /**
   * @param tokenSuffix to uniquely identify and separate this token from other on the same restClient and {@link ISession session}.
  ...
  /**
   * Sets a rest-client property, that will be used to isolate your auth-token from other calls to the rest-service, that work with the same {@link ISession session}.
   *
   * <p>By default we read the rest-client property {@value #AUTH_SCOPE_PROPERTY}, if it exists and holds a value, it will be used.<br/>
   * You can pass another property name, if you happen to have your own 'scope' property.</p>
   *
   * @param scopeProperty the property of your rest client feature configuration, that holds the "scope" (permissions) to ask for.
   * @return this instance, for fluent building
   * @since 10.0.3
   */
  @PublicAPI
  ...
  private String getNewAccessToken(Client client, FeatureConfig config) {
    return getAccessToken(client, config, null);
  }

  private String getRefreshedAccessToken(Client client, FeatureConfig config, String refreshToken) {
    return getAccessToken(client, config, refreshToken);
  }

  private String getAccessToken(Client client, FeatureConfig config, String refreshToken) {
    GenericType<Map<String, Object>> map = new GenericType<>(Map.class);
    var tokenUri = uriFactory.getTokenUri();
    String authCode = getAuthCode();
    var authContext = new AuthContextNew(
    ...
    if (response.getStatusInfo().getFamily() == Family.SUCCESSFUL) {
    	Map<String, Object> m = response.readEntity(map);
    	return new String(m.get("access_token").toString());
    }
    throw authError()
        .withMessage("Failed to get or refresh access token: " + response)
